...
async function applyTasksRLSFix() {
  console.log('Applying RLS fix for tasks table...\n');

  // Read the SQL file
  const sql = fs.readFileSync('supabase/migrations/20241029000006_fix_tasks_rls_permissions.sql', 'utf8');
  ...
  // Test if we can query tasks now
  ...
  if (testError) {
    ...
    console.log('   Go to: Database > Tables > tasks > RLS Policies');
  } else {
    console.log('✓ Tasks table is now accessible!');
  }
